...
class SessionManager {
    ...
    async resolve() {
        ...
        if (creds.registered !== true) {
            // This is not a hard failure — Baileys *can* sometimes finish
            // registration on the very next connect — but in practice a
            // creds blob captured with registered:false almost always means
            // the linking flow reported "connected" before WhatsApp actually
            // finished confirming the device, and the very next real
            // connection gets rejected with a 401 (loggedOut). This matches
            // a known intermittent WhatsApp/Baileys pairing issue rather
            // than anything wrong with this bot's code.
            console.log(
                '\x1b[33m⚠️  This session has registered:false — it was captured before WhatsApp ' +
                'fully confirmed the device link. It may fail with a 401 immediately on connect.\x1b[0m\n' +
                '\x1b[33m   If that happens: generate a fresh session, and before copying the string, ' +
                'confirm on your phone under WhatsApp → Settings → Linked Devices that the bot device ' +
                'actually appears there and stays there for at least 30–60 seconds — not just that the ' +
                'linking page says "connected".\x1b[0m'
            );
        }
