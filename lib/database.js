...
class Database {
    ...
    // ---- Runtime settings (presence mode, status emojis, etc.) ----
    // These are config values that get changed via WhatsApp commands
    // (.presence, .statusemojis, .statusreact, .mode, .setprefix) and need
    // to survive a restart, unlike the rest of the in-memory config which
    // resets to .env defaults.
    getRuntimeSettings() {
        return this.data.settings.runtime || {};
    }

    setRuntimeSetting(key, value) {
        if (!this.data.settings.runtime) this.data.settings.runtime = {};
        this.data.settings.runtime[key] = value;
        this.save();
    }
}
